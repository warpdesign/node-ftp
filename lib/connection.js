...
FTP.prototype.list = function(path, zcomp, cb, options) {
  var self = this, cmd, auto = false, encoding = 'auto';

  if (typeof path === 'function') {
    // list(function() {}, options)
    if (zcomp && zcomp.encoding)
      encoding = zcomp.encoding;
    cb = path;
    path = undefined;
    cmd = 'LIST';
    zcomp = false;
  } else if (typeof path === 'boolean') {
    // list(true, function() {}, options)
    if (cb && cb.encoding)
  ...
  console.log('list encoding options', auto, encoding, arguments);
  ...
  this._pasv(function(err, sock) {
    ...
    source.on('data', function(chunk) {
      ...
      // guess the server doesn't support utf8, switch to latin1 (todo: better sol here)
      if (auto && decoder.encoding === 'utf8' && str.match('�')) {
        decoder = new StringDecoder('latin1');
        console.log('using latin decoder !');
        buffer += decoder.write(chunk);
      } else {
        buffer += str;
      }
